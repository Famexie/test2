...
static void APP_HFP_AudioTransferKeyHandler(BD_ADDR_T *pBdAddr, BOOL isViaKey)
{
	...
	if (APP_IsActiveLink(pBdAddr))
	{
		...
		{
			DBG_LOG_APP_HFP( "[APP_HFP_KEY] Audio Transfer From AG change to DUT", 0);
			APP_HFP_CreateAudioConnection(pBdAddr, isViaKey);
		}
		else
		{
			DBG_LOG_APP_HFP( "[APP_HFP_KEY] Audio Transfer From DUT change to AG", 0);
...
*/
	...
	{
		DBG_LOG_APP_HFP( "[APP_HFP_KEY] !APP_IsActiveLink", 0);
/*
