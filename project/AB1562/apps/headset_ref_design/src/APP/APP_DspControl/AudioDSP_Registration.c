...
static void app_SetSuspendA2dpLink(BD_ADDR_T *pBdAddr)
{
	FW_Memcpy(&gAppAudioDevCtl.suspendA2dpBdAddr, pBdAddr, sizeof(BD_ADDR_T));
}

static void app_ClearSuspendA2dpLink(void)
{
	FW_Memset(&gAppAudioDevCtl.suspendA2dpBdAddr, 0, sizeof(BD_ADDR_T));
}

static BOOL app_IsResumeSuspendedA2dpLink(BD_ADDR_T *pBdAddr)
{
	if(FW_IsBdAddrZero(&gAppAudioDevCtl.suspendA2dpBdAddr))
		return FALSE;

	return !FW_CmpBdAddr(&gAppAudioDevCtl.suspendA2dpBdAddr, pBdAddr);
}
...
static BOOL app_WhetherSetScoAudioDev(BD_ADDR_T *pBdAddr)
{
	...
	if(APP_State_CheckLinkInCallActiveState(pCurrentActiveLink)) //maybe APP_CALL_ACTIVE or APP_CALL_HELD
	{
		#ifdef HFP_Profile
		//if(APP_MAX_ONGOING_CALL_NO == APP_HFP_SearchCallStatusInd(pCurrentActiveLink, APP_CALL_HELD))
		if(APP_MAX_ONGOING_CALL_NO != APP_HFP_SearchCallStatusInd(pCurrentActiveLink, APP_CALL_ACTIVE)) //if it is APP_CALL_ACTIVE
			return FALSE;
		#endif
...
BOOL APP_AudioDeviceRegister(BD_ADDR_T *pBdAddr, U8 audioDev)
{
	...
	DBG_LOG_APP_DSP_CONTROL( "[Audio Reg] current dev:%d, in dev:%d, current pri:%d, in pri:%d", 4,
		currentActiveDev, audioDev, priCurr, priIn);
...
void APP_AudioDeviceDeRegister(BD_ADDR_T *pBdAddr, U8 audioDev)
{
	...
	if(audioDev == AUDIO_DEVICE_SCO || audioDev == AUDIO_DEVICE_SCO_HSP || audioDev == AUDIO_DEVICE_SCO_IDLE || audioDev == AUDIO_DEVICE_CALL_STATE|| audioDev == AUDIO_DEVICE_KEY_AT_CMD )
	{
		if(!APP_A2dp_NVKeyIsFeatureOn(APP_A2DP_MEMPUT_WHEN_CALLACTIVE_FEAT) && app_IsResumeSuspendedA2dpLink(pBdAddr) && !APP_SCO_EXISTS(pBdAddr)
			&& !APP_State_CheckLinkInCallRelatedState(pBdAddr) && APP_IsActiveLink(pBdAddr))
		{
			APP_A2dp_SendResume(&gAppAudioDevCtl.suspendA2dpBdAddr);
			app_ClearSuspendA2dpLink();
		}
	}
	#endif
