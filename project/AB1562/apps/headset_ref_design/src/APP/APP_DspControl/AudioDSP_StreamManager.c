...
static void app_DisableOtherLinkLCPkt(BD_ADDR_T *pBdAddr)
{
	BD_ADDR_T *pNextBdA = APP_GetNextNormalLink(pBdAddr);

	DBG_LOG_APP_DSP_CONTROL("[APP][DSPStreamManager] a2dp disable LC, on:%x%x, off:%x%x", 4, FW_bdaddr_to_2U32(pBdAddr, TRUE), FW_bdaddr_to_2U32(pBdAddr, FALSE), FW_bdaddr_to_2U32(pNextBdA, TRUE), FW_bdaddr_to_2U32(pNextBdA, FALSE));
	if(pNextBdA)
	{
		bt_a2dp_drop_pkt_setting(pNextBdA, TRUE);
	}
}
/******************************************************************************
...
static BOOL app_Audio_OpenPathBgEffect(U8 audio_type)
{
    ...
    switch(audio_type)
    {
        case AUDIO_ANC:
            ...
            if(isSuccessful)
            {
                APP_PowerOff_SetAutoSwitchOffBit(AUTO_SWITCH_OFF_BIT_ANCPASSTHRU, TRUE, APP_GetServiceBdAddr());
            }
            break;
        case AUDIO_ADDA:
...
static BOOL app_Audio_OpenMainPath(U8 audio_type, BD_ADDR_T *pBdAddr, U8 codec, U8 scms, U8 sampleRateIndex)
{
    ...
    switch(audio_type)
    {
        case AUDIO_SCO:
            ...
            isSuccessful = drv_sco_start(pBdAddr, codec, APP_GetSCOHandleByBdAddr(pBdAddr), 0);
            ...
            break;
        case AUDIO_A2DP:
            app_DisableOtherLinkLCPkt(pBdAddr);
            ...
            if(isSuccessful)
            {
                ...
                bt_a2dp_drop_pkt_setting(pBdAddr, FALSE);
                nvkey_hdl_set_state(NVKEY_HDL_STATE_MASK_APP_DSP_MAIN_PATH);
