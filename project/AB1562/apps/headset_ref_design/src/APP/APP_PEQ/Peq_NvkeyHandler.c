...
bool APP_PeqNvkey_IsPeqFeatureOn(void)
{
	PEQ_AUDIO_PATH_TABLE_STRU *pAudioPathTable = (PEQ_AUDIO_PATH_TABLE_STRU *)NVKEY_GetPayloadFlashAddress(NVKEYID_APP_PEQ_AUDIO_PATH_TABLE);
	
	if(pAudioPathTable)
	{
		return (pAudioPathTable->numOfAudioPath) ? TRUE : FALSE;
	}
