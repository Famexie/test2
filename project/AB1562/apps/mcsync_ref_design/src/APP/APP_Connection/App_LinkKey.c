...
U8 APP_LinkKey_GetNormalLinkNum(void)
{
	return gLinkKeyCtl.normalLinkNum;
}
...
void APP_LinkKey_ClearAllHistory(void)
{
    ...
    if(APP_FCD_IsFcdReset()) //only FCD command
    {
        FW_Memset(gLinkKeyCtl.pFcdHistoryInfo, 0, sizeof(LINK_HISTORY_STRU));
        app_linkkey_write_nvkey_fcd_link();
    }

    APP_MCSYNC_SendSyncClearLinkKey();
}
...
U8 APP_LinkKey_AddNormalByHistoryInfo(LINK_HISTORY_STRU *plinkListIn)
{
	...
	plinkListDest = APP_LinkKey_SearchHistoryByBdAddr(&plinkListIn->linkData, FALSE, FALSE);
	...
	return APP_LinkKey_SearchLinkKeyIndex(&plinkListIn->linkData, FALSE);
}
