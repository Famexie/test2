...
static void app_SystemKey_FollowerStopsShareMode(BD_ADDR_T * pBdAddr);
static void app_SystemKey_AgentToggleShareMode(BD_ADDR_T * pBdAddr);
static void app_SystemKey_FollowerToggleshareMode(BD_ADDR_T * pBdAddr);
#endif
...
static void app_SystemKey_PowerOnDiscoverable(BD_ADDR_T *pBdAddr)
{
	...
	ChgCase_Smart_KeyTriggerInitState();
...
static void app_SystemKey_ResetToFactory(BD_ADDR_T *pBdAddr)
{
	...
	if(BtMCSync_IsFeatureSupport(AWSMCE_SYNC_FACTORY_RESET_ROLE_FEAT))
	{
		BtAwsMce_WriteDefaultInfo(ROLE_NONE, NULL, NULL);
		if(APP_Conn_GetAgLinkState() == LINK_DISCONNECTED)
		{
...
{
	UNUSED(pBdAddr);
	APP_MCSYNC_SendSyncVPInfo(TRUE);
	APP_VpEnableVoicePrompt();
}
...
static void app_SystemKey_VoicePromptDisable(BD_ADDR_T *pBdAddr)
{
	...
	APP_MCSYNC_SendSyncVPInfo(FALSE);
	APP_VpDisableVoicePrompt();
}
...
static void app_SystemKey_AiroThru(BD_ADDR_T *pBdAddr)
{
	...
	if(APP_Customer_isNeedSaveANCPassThruState(FALSE))
	{
...
#ifdef AB1562F
	para.type = HAL_AUDIO_ANC_TYPE_FF;
#else
...
static U16 app_SystemKey_CheckKeyDiscoverable(U16 keyEventCode)
{
	if(MCSYNC_IsCurrentRolePartner() && (MCSYNC_LINK_CONNECTED == MCSYNC_GetMcsyncState() || APP_GetAGNum() >= APP_Conn_GetSupportDeviceNo()))
    {
        keyEventCode = KEY_INVALID;
    }
...
static U16 app_SystemKey_CheckKeyVoiceUpDown(U16 keyEventCode)
{
	...
	if(!APP_AudioDspInMainPathStreaming() && !APP_AudioDspIsStreaming(NULL, AUDIO_ANC))
		keyEventCode = KEY_INVALID;
