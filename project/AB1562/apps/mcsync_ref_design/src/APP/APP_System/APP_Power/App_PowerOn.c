...
void APP_PowerOn_Loader(void)
{
	...
	if(APP_PowerOff_IsPowerOn())
	{
		...
		BtModuleLog_SendLogHandler(MODULE_LOG_MODULE_APP_POWER, 
			MODULE_LOG_MODULE_APP_POWER_SUB_MODULE_POWER_ON, 0);
		#endif
		return;
	}
...
*/
	...
	{
		BOOL status = APP_MCSync_Rho_Stop();
		DBG_LOG_APP_SYSTEM( "[APP_PWRON] RHOing. Stop status:%d", 1, status);
	}
	...
	APP_Customer_SetSmartChargeStateWhenPowerOn();

	if(BtAwsMce_IsDefaultRoleAgent() || (BtAwsMce_IsDefaultRolePartner() && BtMCSync_IsFeatureSupport(AWSMCE_AUTO_RECONN_AFTER_PWR_ON_FEAT)))
	{
	...
	APP_Customer_PowerOnLoaderHandler();
...
//	DRIVER_ResetLineInKeyTimer();
	...
	APP_AWSMCE_SetTimer(APP_MCS_TIMER_AIR_PAIRING, 5000);
	#endif
	...
	#ifdef MODULE_LOG_FEATURE
	BtModuleLog_SendLogHandler(MODULE_LOG_MODULE_APP_POWER, 
	MODULE_LOG_MODULE_APP_POWER_SUB_MODULE_POWER_ON, 1);
	#endif
