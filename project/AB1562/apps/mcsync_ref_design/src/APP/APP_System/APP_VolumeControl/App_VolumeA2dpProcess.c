...
void APP_VolA2dp_VolumeUpDown(BD_ADDR_T *pBdAddr, BOOL isVolumeUp, BOOL isFollowerCmd)
{
	...
	if(!isFollowerCmd)
	{
		...
		{
			APP_VolProcess_Beep(soundLevel, AUDIO_DEVICE_MUSIC, TRUE);
		}
		else
		{
	...
	#endif
	{
		app_VolA2dp_SetLink(pBdAddr, soundLevel, isLocal);
...
void APP_VolA2dp_SyncBeep(BD_ADDR_T *pBdAddr)
{
	APP_VolProcess_Beep(APP_VolA2dp_GetCurrentSoundLevel(pBdAddr), AUDIO_DEVICE_MUSIC, FALSE);
}
