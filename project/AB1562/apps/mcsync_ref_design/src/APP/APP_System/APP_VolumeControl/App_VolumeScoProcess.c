...
static void app_VolSco_SendSpkMuteOnOffToDSP(bool spkMuteOn)
{
	spkMuteOn ? APP_VolSetting_SetSpeakerMuteOn(SPKR_MUTE_MASK_SCO) : APP_VolSetting_SetSpeakerMuteOff(SPKR_MUTE_MASK_SCO) ;
}
...
void APP_VolSco_VolumeUpDown(BD_ADDR_T *pBdAddr, BOOL isVolumeUp, BOOL isFollowerCmd)
{
	...
	app_VolSco_SetLink(pBdAddr, soundLevel, isLocal);
...
void APP_VolSco_SyncBeep(BD_ADDR_T *pBdAddr)
{
	APP_VolProcess_Beep(APP_VolSco_GetCurrentSoundLevel(pBdAddr), AUDIO_DEVICE_SCO, FALSE);
}
