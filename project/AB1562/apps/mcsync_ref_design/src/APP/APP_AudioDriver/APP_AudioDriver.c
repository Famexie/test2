...
void APP_AudioDriver_SendRingtoneCmd(U16 id, U16 srcIndex, BOOL isPlayNow)
{
	if(isPlayNow)
	{
		APP_VpRt_SetMediaEventInfo(id, 0xFF, srcIndex, 0);
		APP_VpRt_OSDPCTimer((id == MEDIA_EVT_PLAYING_BEEP_SYNC) ? AWSMCE_VPRT_BEEP_TIMER : AWSMCE_VPRT_PLAYING_TIMER, APP_VP_RT_DIRECTLY_PLAY_TIME);
	}
