...
extern APP_STATE_HANDLER_SET_TYPE_STRU const gAppStateMCSyncHandler;
#ifdef LINEIN_ENABLE
...
const APP_STATE_HANDLER_SET_TYPE_STRU const *gAppStateHandlerSet[APP_TOTAL_STATE_NO] =//MMI_StateHandlerSet
{
	...
	&gAppStateMCSyncHandler, //APP_TWS_PAIRING
	NULL, //APP_INQUIRY
	NULL, //APP_MP_STATE
	NULL, //APP_MP3
};
...
static U32 app_State_TimerHandler(Handler handler, U16 id, void *payload, U32 id_ext)
{
	...
	switch(id)
	{
		...
		case PREVENT_AUTO_POWER_ON_TIMER:
			...
			{
				DBG_LOG_APP_STATE( "[APP_STATE] APP_ChgBat_IsInChargerCase():%d", 1, APP_ChgBat_IsChargerIn());
			...
			else
			{
				...
				else if(APP_AirApp_FOTA_Reconnect())
				{
					break;//BTA-3762
				}
				else if(!APP_ChgBat_IsChargerIn())
				{
					if(drv_rt_is_playing() || drv_vp_is_playing())
					{
						DBG_LOG_APP_STATE( "[APP_STATE] Vp/Rt is alive, set PREVENT_AUTO_POWER_ON_TIMER", 0);
						APP_State_SetTimer(PREVENT_AUTO_POWER_ON_TIMER, 1 * MILLI_SEC_100);
					}
					else
					{
						//APP_AudioDspClose();
		...
		case RECONN_FAIL_ENTER_DISC_TIMER:
			{
				...
				if((mcsyncState != MCSYNC_LINK_CONNECTED) && (connState != LINK_CONNECTED))
				{
...
static void app_State_LoadState(BD_ADDR_T *pBdAddr, U8 state)
{
	...
	if(state == topState || (BtAwsMce_IsDefaultRoleAgent() && APP_GetAGNum() == 0) )
	{
...
static void app_State_OffHandler(BD_ADDR_T *pBdAddr, APP_MSG_TYPE_STRU *pMsg)
{
	...
	switch(pMsg->eventCode)
	{
		case APP_FAKE_MEDIA_EVT:
			...
			else if(pMsg->msgOpcode == MEDIA_EVT_POWER_OFF)
			{
				APP_AudioDspClose();
		...
		case APP_CHECK_POWERON_TIMER:
			{
				...
				if(SYS_BOOT_REASON_STATUS() == BOOT_CODE_POWEROFF_FAIL_RESET)
				{
					APP_PowerOff_SystemOff();
					break;
				}
				...
				{
					if(SYS_BOOT_REASON_STATUS() == BOOT_CODE_HARDWARE_RESET)
					{
						if (ChgCase_Smart_IsCaseSmartType() && (ChargerSmartCase_GetSmartCaseState() == STATE_CASE_OFF))
							ChargerSmartCase_SetSmartCaseState(STATE_OUT_OF_CASE);
					}
					app_State_FastPowerOn(FALSE);
					break;
				}
...
static bool app_State_InsertStateByLink(BD_ADDR_T *pBdAddr, U8 state, U8 beforeState)
{
	...
	{
		pLinkInfo->pNestState = pNewState;
		if (app_State_ReadStateType(state) == TYPE_ONE_LINK)
		{
...
static bool app_State_InsertStateAllLink(U8 state, U8 beforeState)
{
	...
	for (linkIndex = 0; linkIndex < FW_MAX_LINK_IDX; linkIndex++)
	{
		...
		if (pPreviousState == pCurrentState) //the before state is the top state
		{
			...
			if (app_State_ReadStateType(state) == TYPE_ONE_LINK)
			{
				APP_MCSYNC_SendSyncStateInfo(&gAppCtl.appInfo[linkIndex].bdAddr);
...
U8 APP_State_CountConnectedDev(void)
{
	...
	for(linkIndex = 0; linkIndex < FW_MAX_LINK_IDX; linkIndex++)
	{
		...
		if(pBdAddr && APP_State_CheckNestStateByLink(pBdAddr, APP_CONNECTED)
		   && !BtAwsMce_IsSpecialLink(pBdAddr)
		  )
		{
			count++;
		}
	...
	if(APP_PowerOff_IsPowerOn())
	{
		...
		#endif
		{
			...
			else
			{
				...
				{
					APP_Media_PushMediaEvent(MEDIA_EVT_SPECIAL_DISCONNECTED);
...
void APP_State_LoadDefaultTopState(void)
{
	app_State_LoadState(APP_EOF, app_State_GetTopStateInDefaultLink());
}
