	...
	if(AMA_Target_IsEnable())
	{
		if(APP_Ama_GetLinkAVRCPOverride())
		{
			...
			if(pAmaLinkInfo)
			{
				...
				{
					APP_Avrcp_SetTimer(pBdAddr, 
						APP_AVRCP_TIMER_AMA_MEDIA_CONTROL_PLAY, 1 * ONE_SEC, 0);
				}
			}
			//return;
		}
...
static void app_AvrcpRegisterNotifyRspIndHandler(AVRCP_GET_REGISTER_NOTIFY_IND_T *msg)
{
	...
	switch (msg->eventId)
	{
		case AVRCP_EVENT_ID_PLAYBACK_STATUS_CHANGED:
		{
			...
			if(pLinkInfo)
			{
				DBG_LOG_APP_AVRCP("[APP InEarDetection] msg->params.playStatus(%d)(%d)", 2, msg->params.playStatus, pLinkInfo->playStatus);
				pLinkInfo->isDisablePlayStatusMonitor = TRUE;
				if(APP_InEarDetection_IsEnable())
				{
					if(msg->params.playStatus == AVRCP_PLAYBACK_STATUS_PLAYING && pLinkInfo->playStatus != AVRCP_PLAYBACK_STATUS_PLAYING)
					{
						DBG_LOG_APP_AVRCP("[APP InEarDetection] playStatus change to play, clear pause flag", 0);
...
U8 APP_Avrcp_GetMsgNumInQueue(void)
{
	return MSG_MessageSearchAllHandler((Handler)&gAppAvrcpHandle);
}
