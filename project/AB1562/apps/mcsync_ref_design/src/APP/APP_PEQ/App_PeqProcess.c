...
static void app_PeqProc_PeqGroupCalculation(U8 peqComponent)
{
	...
	APP_Customer_PEQVpToggle(groupIndex);
...
BOOL APP_PeqProc_FillRhoData(void *pData)
{
	APP_RHO_PEQ_CTL_STRU *pDataToFill = &((APP_RHO_DATA_STRU *)pData)->appPEQ;

	if(!pDataToFill)
	{
		DBG_LOG_APP_PEQ( "[APP_PEQ] Fill RHO data error", 0);
		return FALSE;
	}

	FW_Memcpy(pDataToFill, gAppPeqCtl.PEQGroup, sizeof(U8)*PEQ_PATH_NUM);

	return TRUE;
}

BOOL APP_PeqProc_AssignRhoData(void *pData)
{
	APP_RHO_PEQ_CTL_STRU *pDataToGet = &((APP_RHO_DATA_STRU *)pData)->appPEQ;

	if(!pDataToGet)
	{
		DBG_LOG_APP_PEQ( "[APP_PEQ] Assign RHO data error", 0);
		return FALSE;
	}

	FW_Memcpy(gAppPeqCtl.PEQGroup, pDataToGet, sizeof(U8)*PEQ_PATH_NUM);
