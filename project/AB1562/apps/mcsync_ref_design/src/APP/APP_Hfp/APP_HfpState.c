...
static void app_HFP_ConnectedHandler(BD_ADDR_T *pBdAddr, APP_MSG_TYPE_STRU *pMsg)//(U8 linkIndex, MMIMsgType XDATA_PTR msgPtr)
{
	...
	switch(pMsg->eventCode)
	{
		...
		case APP_FAKE_MEDIA_EVT:
			if(pMsg->msgOpcode == MEDIA_EVT_PLAYING_BEEP_SYNC)
			{
				BOOL isSentVGS = FALSE;
				BOOL isSendAVRCP = FALSE;
				if(APP_IsActiveAudioDevInCall())
				{
					isSentVGS = TRUE;
				}
				#ifdef AVRCP_Profile
				else if(APP_IsActiveAudioDevInMusic())
				{
					isSendAVRCP = TRUE;
				}
				#endif
...
#ifdef PROFILE_HEADSET_ENABLE
				if(isSentVGS && (APP_Hfp_IsConnected(pBdAddr) || APP_Hfp_IsHspConnected(pBdAddr)))
#else
                if(isSentVGS && APP_Hfp_IsConnected(pBdAddr))
#endif                    
					APP_HFP_SendATCmdByLink(pBdAddr, HFP_ATCMD_ADJUST_VOLUME);
				#ifdef AVRCP_Profile
				if(isSendAVRCP)
					APP_Avrcp_SendVolumeChangeNotification(pBdAddr, APP_VolManage_GetCurrentA2dpSoundLevel(pBdAddr));
				#endif
			}
			else if(state == APP_CONNECTED || (APP_State_IsInSystemAudioState() && APP_State_CheckNestStateByLink(pBdAddr, APP_CONNECTED)))
			{
...
static void app_HFP_ImgEntry(BD_ADDR_T *pBdAddr)//(U8 linkIndex)
{
	...
	// add disable sniff code
	App_SniffDisable(pBdAddr, APP_SNIFF_HFP);
	APP_ImgMedia_AudioVoiceHandler(pBdAddr, IMG_AUDIO_VOICE_TRIGGER);
...
static void app_HFP_ImgExit(BD_ADDR_T *pBdAddr)//(U8 linkIndex)
{
	...
	App_SniffEnable(pBdAddr, APP_SNIFF_HFP);
}
