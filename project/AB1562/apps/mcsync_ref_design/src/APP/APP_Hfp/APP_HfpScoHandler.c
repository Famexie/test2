...
#define APP_HFP_SCO_STOP_CFM    (BT_MSG_E_MAX + 1)
...
static BOOL gAppHfpScoStopNotify;
/**************************************************************************************************
* Structure
**************************************************************************************************/
/*Static*/
...
static void app_HfpSCO_DiscHandle(BD_ADDR_T *pBdAddr)
{
	...
	{
		FW_MessageSend((Handler)&gAppHfpSCOHandle, APP_HFP_SCO_STOP_CFM, NULL);
	}
...
static void app_HfpSCO_DiscProfileHandle(BD_ADDR_T *pBdAddr)
{
	...
	{
		DBG_LOG_APP_HFP( "[APP_HFP] release and detach", 0);
...
static void app_HfpSCOStopCfmHandler(void)
{
	AUDIO_DSP_STOP_PARA_STUR audioDspPara;

	DBG_LOG_APP_HFP( "[APP_HFP] Enter SCO stop handler", 0);
    
    FW_Memset(&audioDspPara, 0, sizeof(AUDIO_DSP_STOP_PARA_STUR));
    audioDspPara.audioType = AUDIO_SCO_DISC;
    APP_AudioDspStop(&audioDspPara);
	
	if(APP_A2DP_GetDspStartHandler() != (Handler)NULL)
	{
		DBG_LOG_APP_HFP( "[APP_HFP] call a2dp start handler", 0);
		FW_MessageSend(APP_A2DP_GetDspStartHandler(), 0, NULL);
	}
}
...
static U32 app_HfpSCOHandler(Handler handler, U16 id, void *msg, U32 handler_id)
{
	...
	switch(id)
	{
        ...
        case APP_HFP_SCO_STOP_CFM:
            app_HfpSCOStopCfmHandler();
            break;
...
void APP_HfpSco_SendDSPCmd(BD_ADDR_T *pBdAddr)
{
    ...
    {
        if(APP_GetAgentScoState() != SCO_STATE_CONNECTED)
        {
            return;
        }
    }
...
void APP_HfpSco_DisconnectHandleByBdAddress(BD_ADDR_T *pBdAddr)
{
	...
	BtSyncSetRetransmitssionEnable((Handler)&gAppHfpSCOHandle,&pBdAddr->addr,0);
}
...
void APP_HfpSco_SetRetransmissionEnable(BD_ADDR_T *pBdAddr, U8 enable)  // 1:disable 0:enable
{
	...
	if(isSCOExist)
	{
		DBG_LOG_APP_HFP( "[APP_HFP] BtSyncSetRetransmitssionEnable %d", 1, enable);
		BtSyncSetRetransmitssionEnable((Handler)&gAppHfpSCOHandle,&pBdAddr->addr,enable);
	}
}

void APP_HfpSco_RegisterStopNotify(void)
{
    DBG_LOG_APP_HFP( "[APP_HFP] RegisterScoStopHandler", 0);
	gAppHfpScoStopNotify = TRUE;
}

void APP_HfpSco_DeRegisterStopNotify(void)
{
    DBG_LOG_APP_HFP( "[APP_HFP] DeRegisterScoStopHandler", 0);
	gAppHfpScoStopNotify = FALSE;
}

BOOL APP_HfpSco_IsRegisterStopNotify(void)
{
    return gAppHfpScoStopNotify;
}

U8 APP_HfpSco_GetMsgNumInQueue(void)
{
	return MSG_MessageSearchAllHandler((Handler)&gAppHfpSCOHandle);
}
